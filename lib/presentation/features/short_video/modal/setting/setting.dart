...
class SettingData {
  ...
  SettingData.fromJson(dynamic json) {
    ...
    _coinValue = json['coin_value'] is int
        ? double.parse("${json['coin_value']}")
        : json['coin_value'];
...
class Gifts {
  ...
  // Phương thức chuyển đổi danh sách JSON sang danh sách Gifts
  static List<Gifts> toList(List<dynamic> jsonList) {
    return jsonList.map((json) => Gifts.fromJson(json)).toList();
  }
}
