...
import 'widget/add_btn_sheet.dart';
import 'widget/bottom_input_bar.dart';
import 'widget/chat_area.dart';
import 'widget/image_video_msg_screen.dart';
import 'widget/top_bar_area.dart';
...
class _ChatScreenState extends State<ChatScreen> {
  ...
  String dateFormat(DateTime time) {
    ...
    final bool isAm = (time.hour * 60) > 719 ? false : true;
    final String timeStr = '$hour:$minute ${isAm ? 'am' : 'pm'}';
    return timeStr;
  }
  ...
  Future<void> firebaseMsgUpdate(
    ...
    if (chatData.isEmpty && deletedId.isEmpty) {
      final Map con = conversation?.toJson() ?? {};
  ...
  void dispose() {
    drSender
        ...
        .then(
      (value) {
        final senderUser = value.data()?.user;
