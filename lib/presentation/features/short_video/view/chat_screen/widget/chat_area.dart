...
class ChatArea extends StatelessWidget {
      ...
      return Expanded(
        child: ListView.builder(
          ...
          itemBuilder: (context, index) {
            final String date = chatData?.keys.elementAt(index) ?? '';
            final List<ChatMessage>? messages = chatData?[date];
            return Column(
              children: [
  ...
  Widget yourMsg(
      ...
      ChatMessage? data}) {
    final bool selected = timeStamp.contains('${data?.time?.round()}');
    final bool isMe = data?.senderUser?.userid == SessionManager.userId;
    return InkWell(
      onLongPress: () {
        onLongPress(data);
      },
      ...
      child: Container(
        ...
        child: Row(
          ...
          children: [
            isMe ? _dateTimeMessage(data) : const SizedBox(),
